...
export default function HomePage() {
  const { selectedNeighborhood, setSelectedNeighborhood } = useNeighborhood()
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
              <div className="flex items-center gap-2 rounded-lg border bg-background p-1 shadow-sm">
                <Select value={selectedNeighborhood} onValueChange={setSelectedNeighborhood}>
                  <SelectTrigger className="w-[250px] border-0 focus:ring-0">
                    <MapPin className="mr-2 h-4 w-4 text-primary" />
                    <SelectValue placeholder="Scegli quartiere" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">Scegli il tuo quartiere</SelectItem>
                    {neighborhoods.map((n) => (
        ...
        <section id="starter-kits">
          <div className="mb-10 text-center">
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl">
              Scegli il tuo <span className="text-primary">starter kit</span>
            </h2>
            <p className="mt-2 text-muted-foreground">Idee pronte all&apos;uso per attivare la tua comunità</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {displayKits.map((kit) => (
              <StarterKitCard key={kit.id} kit={kit} customUrl="/grazie"/>
            ))}
          </div>
        </section>
      </main>
      ...
      <section id="come-funziona" className="border-y bg-muted/30 py-16 md:py-24">
