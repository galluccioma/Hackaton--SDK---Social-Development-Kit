...
export default function HomePage() {
  ...
  const filteredStarterKits = useMemo(() => {
    return starterKits.filter((kit) => {
      if (
        ...
        kit.category !== filters.activityType
      )
        return false;
      if (filters.difficulty !== "all" && kit.difficulty !== filters.difficulty)
        return false;
      return true;
    });
  }, [filters]);
  ...
  const filteredProjects = useMemo(() => {
    return pendingProjects.filter((project) => {
      if (
        filters.activityType !== "all" &&
        project.category !== filters.activityType
      )
        return false;
      if (
        selectedNeighborhood !== "all" &&
        project.neighborhood !== selectedNeighborhood
      )
        return false;
      if (
        project.requiredCollaborators < filters.collaborators[0] ||
        project.requiredCollaborators > filters.collaborators[1]
      )
        return false;
      return true;
    });
        ...
        <div className="container mx-auto px-4 py-16 md:py-24">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mb-8 text-lg text-muted-foreground md:text-xl text-pretty">
              Scarica uno starter kit, trova collaboratori e lancia attività che
              uniscono le persone. Tutto parte da un&apos;idea condivisa.
            </p>
              ...
              <div id="projects" className="mb-12">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-semibold">
                    Progetti in cerca di collaboratori
                  </h2>
                  <Link
                    href="#"
